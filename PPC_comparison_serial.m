    ...
    for iS = 1:length(S_list)
        ...
        for iShuf = 1:nShuf
            ...
            for iT = 1:length(data_i.trial) % shuffle each trial separately 
                % consider circ_shift version instead.  this would maintain
                % train relationships and offset relative to the csc.  Can
                % use randsom set of shift values (set all offset values,
                % randomize and pick some number of them)
